#include "ca/cc/glcore/buffer.hpp"
...
namespace ca { namespace cc {
		...
		m_mappedSize(0)
	{
		...
		pa::logPedantic("[ca::cc] Created buffer ", m_id, " with size ", m_size, ".");
	}
		...
		Buffer(_type, _elementSize, _numElements, _usageBits, _data)
	{
		...
		pa::logPedantic("[ca::cc] Created texture view ", m_tbID, " for the buffer ", m_id, ".");
	}
	...
	Buffer::~Buffer()
	{
		...
		pa::logPedantic("[ca::cc] Deleted buffer", m_id, " (w/o texture view", m_tbID, ").");
	}
	...
	void Buffer::subDataUpdate(GLintptr _offset, GLsizei _size, const GLvoid* _data)
	{
		if(!(m_usage & Usage::SUB_DATA_UPDATE)) {
			pa::logError("[ca::cc] Buffer::subDataUpdate requires Usage::SUB_DATA_UPDATE. The current buffer is static.");
			return;
		}
